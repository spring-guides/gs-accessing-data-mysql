...
We here begin with either `create` or `update` because we don't have the database structure yet. After the first run, we could switch it to `update` or `none` according to program requirements. Use `update` when you want to make some change to the database structure.
...
NOTE: The above example explicitly specifies `POST` and `GET` for the two endpoints. `@RequestMapping` maps all HTTP operations by default.
...
Now you have 2 HTTP endpoints you can test

`GET localhost:8080/demo/all` This gets all data
`POST localhost:8080/demo/add` This adds one user to the data
